    ...
    for (let i = 1; i < ratings.length; i++) {
      if (ratings[i] !== -1) {
        post("/api/user/comment", {
          creator: props.creator,
          activity_id: props.activity_id,
          member_id: props.members[i - 1].u_id,
          rating: ratings[i],
          comment: reviews[i],
        })
          ...
          <form onSubmit={handleSubmit}>
            ...
            {props.members.map((member, index) => (
              <div key={index + 1}>
                <input
                  type="number"
                  placeholder={`${member.name}打分`}
                  onChange={(event) => {
                    handleRatingChange(event, index + 1);
                  }}
                />
                <input
                  type="text"
                  placeholder={`${member.name}评价`}
                  onChange={(event) => {
                    handleReviewChange(event, index + 1);
                  }}
                />
              </div>
            ))}
