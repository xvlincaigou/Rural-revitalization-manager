        ...
        {activityComments.length === 0 ? (
          ...
          activityComments.map((comment, index) => {
            return (
              <label
                key={index}
              >{`${comment.creator.name}对活动评分：${comment.rating}，评论：${comment.comment}`}</label>
            );
          })
        )}
        <h3>成员评论</h3>
        {userComments.length === 0 ? (
          ...
          userComments.map((comment, index) => {
            return (
              <label
                key={index}
              >{`${comment.content.creator.name}对${comment.to_whom}评分：${comment.content.rating}，评论：${comment.content.comment}`}</label>
            );
          })
        )}
      </Dialog>
