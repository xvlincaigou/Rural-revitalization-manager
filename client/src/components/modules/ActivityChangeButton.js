...
const ActivityChangeButton = (props) => {
  ...
  const [last_register_time, setLast_Register_time] = useState(
    convertToBeijingTime(props.latest_register_time)
  );
  ...
  const handleSubmit = (event) => {
    ...
    const new_date = {
      start: convertToUTC(start_time),
      end: convertToUTC(end_time),
      sign_up: convertToUTC(last_register_time),
    };
    post("/api/activity/update", {
      activity_id: props.a_id,
      location: location,
      intro: information,
      name: name,
      date: new_date,
        ...
        <div className="ActivityDialogue">
          <form onSubmit={handleSubmit}>
            ...
            <input
              type="datetime-local"
              placeholder="最晚报名时间"
              onChange={handleLast_Register_TimeChange}
              value={last_register_time}
            />
            <label>地点</label>
            <input
              type="text"
