...
const SingleActivity = (props) => {
  ...
  useEffect(() => {
    ...
    let inorout =
      usersSignedUpIds.includes(props.user.u_id) || supervisorsIds.includes(props.user.u_id);

    if (latest_register_time > currentDateTime) {
      //活动可以报名
      setTime(
        <div className="Activity-held-time Activity-held-time-color1">
          {convertToBeijingTime(props.start_time) + " ~ " + convertToBeijingTime(props.end_time)}
        </div>
      );
      ...
      //活动报名结束但是没有开始
      setTime(
        <div className="Activity-held-time Activity-held-time-color2">
          {convertToBeijingTime(props.start_time) + " ~ " + convertToBeijingTime(props.end_time)}
        </div>
      );
      setButton(2);
      ...
      //活动正在进行
      setTime(
        <div className="Activity-held-time Activity-held-time-color3">
          {convertToBeijingTime(props.start_time) + " ~ " + convertToBeijingTime(props.end_time)}
        </div>
      );
      setButton(3);
      ...
      //活动结束
      setTime(
        <div className="Activity-held-time Activity-held-time-color4">
          {convertToBeijingTime(props.start_time) + " ~ " + convertToBeijingTime(props.end_time)}
        </div>
      );
      ...
      if (
        props.user.role == 0 &&
        !(supervisorsIds.includes(props.user.u_id) || usersAdminIds.includes(props.user.u_id))
      ) {
        setButton(40);
      } else if (
        props.user.role == 0 &&
  ...
  const handleClick = () => {
    if (button == 11) {
      post("/api/activity/unsubscribe", { uid: props.user.u_id, aid: props._id })
        .then((res) => {
          alert(res.message);
          setButton(10);
          const newUsers_signed_up = users_signed_up.filter(
            (user) => user.u_id !== props.user.u_id
          );
      ...
      post("/api/activity/subscribe", { uid: props.user.u_id, aid: props._id })
        .then((res) => {
          alert(res.message);
          setButton(11);
          setUsers_signed_up([
            ...users_signed_up,
            { u_id: props.user.u_id, name: props.user.name },
          ]);
        })
        .catch((err) => {
          alert(err);
    ...
    <div className="Activity-container">
      ...
      <div className="button-container">
        {
          ...
          button == 10 ? (
            <ActivityRegisterButton inOrOut={false} handleClick={handleClick} />
          ) : button == 11 ? (
            <ActivityRegisterButton inOrOut={true} handleClick={handleClick} />
          ) : button == 40 ? null : button == 41 ? (
            <>
              <ActivityDownloadButton uid={props.user.u_id} aid={props._id} />
              <ActivityRemarkButton
                creator={{ u_id: props.user.u_id, name: props.user.name }}
                activity_id={props._id}
                members={props.users_admin}
              />
            </>
          ) : button == 42 ? (
      ...
      <div className="Activity-infoSection">
        ...
        <div className="Activity-infoBody">
          报名截止时间{" | " + convertToBeijingTime(props.latest_register_time)}
        </div>
