...
const Profile = (props) => {
  ...
  useEffect(() => {
    ...
    get("/api/user/participate_activities", { u_id: props.user.u_id, name: props.user.name })
      .then((res) => {
        //改为返回这个用户对应的活动
        setActivityList(res);
      })
    ...
    get("/api/user/supervise_activities", { u_id: props.user.u_id, name: props.user.name })
      .then((res) => {
        //改为返回这个用户对应的管理的活动
    ...
    get("/api/user/activity_number", { u_id: props.user.u_id })
      .then((res) => {
  ...
  const getRegisterNumber = () => {
    ...
    axios
      .post("/api/user/requst-registration-code", { count: registerNumber })
      .then((response) => {
        ...
        <>
          ...
          <div className="UserManage">
            ...
            <div className="UserManageBlock">
              <input
                type="email"
                placeholder="增删常务管理员，输入用户邮箱"
                onChange={handleAdminUpdate}
              />
            ...
            <div className="UserManageBlock">
              <input
                type="email"
                placeholder="查看用户标签，输入用户邮箱"
                onChange={handleTagUser}
              />
              <UserTagButton
                role={props.user.role}
                u_id={tagUSerEmail}
                operator_id={props.user.u_id}
              />
            </div>
            <div className="UserManageBlock">
              <input
                type="number"
                placeholder="你想获得多少个注册码？"
