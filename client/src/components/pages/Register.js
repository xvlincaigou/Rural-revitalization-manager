...
const Register = ({ upload }) => {
  ...
  const handleLogin = (event) => {
    ...
    if (step === 0) {
      axios
        .post("/api/login", { u_id: email, password: loginpassword })
        .then((response) => {
      ...
      post("/api/login/verifyCode", { u_id: email, code: verificationCode })
        .then((useremailObj) => {
          if (useremailObj) {
            upload(useremailObj);
            // 将useremailObj保存到本地
  ...
  const handleSendCode = (e) => {
    ...
    setCodeCDWarning(false);
    axios
      .post("/api/login/requestCode", { u_id: email })
      .then((response) => {
        ...
        <div className="Register">
          <form onSubmit={handleRegister}>
            ...
            <input
              ...
              onChange={handleIndentificationCardChange}
              required
            />
            <input
              type="password"
    ...
    return (
      <div className="Register">
        <form onSubmit={handleLogin}>
          ...
          {codeCDWarning ? (
            <p className="warning-message">距离上次发送验证码还不足1分钟，请稍后再试。</p>
          ) : null}
          <input
            type="text"
            placeholder="验证码"
            value={verificationCode}
            onChange={handleLoginVerificationCodeChange}
            required
          />
