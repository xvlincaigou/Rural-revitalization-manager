...
exports.register = async (req, res) => {
  try {
    ...
    if (
      settings.availableRegistrationCodes &&
      !settings.availableRegistrationCodes.includes(registration_code)
    ) {
    ...
    if (
      ...
      settings.availableRegistrationCodes = settings.availableRegistrationCodes.filter(
    ...
    if (!passwordIsValid) {
      ...
      if (user.failedLoginAttempts.length === 5) {
        ...
        if (new Date(user.failedLoginAttempts[0].getTime() + 1800000) > now) {
          ...
          return res
            .status(401)
            .send({ message: "密码错误次数过多，您的账户已被锁定，请于24h后再试！" });
        }
      } else {
...
exports.requestCode = async (req, res) => {
  ...
  try {
    ...
    if (
      user.verificationCode.lastSent &&
      new Date(user.verificationCode.lastSent.getTime() + 60000) > new Date()
    ) {
    ...
    await user.save();
    // 发送验证码
    sendCode(user.u_id, verificationCode)
      .then(() => {
...
exports.verifyCode = (req, res) => {
  ...
  User.findOne({ u_id: u_id }).exec(async (err, user) => {
    if (err) {
      res.status(500).send({ message: err });
      return;
    }
    ...
    if (
      ...
      await user.save();
      const token = jwt.sign({ id: user.u_id }, config.secret, {
        algorithm: "HS256",
      ...
      const { name, u_id, role, activities, previous_scores, comment_received, tags, banned } =
        user;
      const strippedUser = {
        name,
        u_id,
      ...
      res.status(200).send(strippedUser);
    } else {
      // 验证码无效或已过期
